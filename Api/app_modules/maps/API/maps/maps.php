<?php
	...
	function UploadXlsxMapsAsync()
	{
		...
		//sleep(20);
		//$RSP_REST['table'] = $obj->conn->get_result_from_query("select * from view_municipio_no_load");
		$obj->conn->execute_single_query("UPDATE localizacion l  SET l.municipio_id = ( SELECT id  FROM municipios mm  WHERE trim(lower( l.municipio_sap )) =  trim(lower( mm.nombre ))  AND l.departamento_id = mm.departamento_id  LIMIT 1);");
		return $RSP_REST;
	}
	...
	function GetnoLoad(){
		...
		$ano_cargue = 2019;
		if (isset($_POST["ano_cargue"])) {
			$ano_cargue = $_POST["ano_cargue"];
		}

		return $obj->conn->get_result_from_query("select * from view_municipio_no_load where ano_carge =  ".$ano_cargue.";");
	}
